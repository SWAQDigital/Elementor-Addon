...
class Portriat_Post_Grid extends Widget_Base { #add ClassName

    public function get_name() {
        return 'Portriat_Post_Grid';
    }

    public function get_title() {
        return __( 'Potriat Post Grid', 'elementor' );
    }
    ...
    protected function _register_controls() {
        ...
        $this->add_control(
            'grid',
            [
                'label' => __( 'Number of Posts Grid', 'swaq' ),
                'type' => \Elementor\Controls_Manager::NUMBER,
                'min' => 1,
                'max' => 2,
                'default' => 2,
...
$args = array('post_type'=>'post', 'post_per_page'=>$settings['posts_per_page'], 'category_name'=>$settings['post_category']);
    ...
    <div><h3 class=""><?php echo $settings['tagline'] ?></h3></div>
    <div><?php if ($settings['show_post_title']) { ?><h1> <strong class=""><a href="<?php the_permalink(); ?>"><?php the_title() ?></a></strong></h1> <?php } ?></div>
    <div><?php if ($settings['show_post_description']) { ?> <p class=""><?php the_excerpt() ?></p> <?php } ?></div>
